...
class FCEFPN(nn.Layer):
    ...
    Args:
        in_channels (list[int]): input channels of each level which can be 
            derived from the model shape of backbone by from_config
        out_channels (list[int]): model channel of each level
        spatial_scales (list[float]): the spatial scales between input feature
            maps and original input image which can be derived from the model
