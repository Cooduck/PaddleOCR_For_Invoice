...
class ChannelReductionEncoder(nn.Layer):
    ...
    Args:
        ...
        out_channels (int): Number of model channels.
    """
...
class SequenceAttentionDecoder(BaseDecoder):
    ...
    Args:
        num_classes (int): Number of model classes :math:`C`.
        ...
        dim_model (int): Dimension :math:`D_m` of the model. Should also be the
            same as encoder model vector ``out_enc``.
        max_seq_len (int): Maximum model sequence length :math:`T`.
        ...
        encode_value (bool): Whether to use the model of encoder ``out_enc``
            as `value` of attention layer. If False, the original feature
            ``feat`` will be used.

    Warning:
        This decoder will not predict the final class which is assumed to be
        `<PAD>`. Therefore, its model size is always :math:`C - 1`. `<PAD>`
    ...
    def forward_train(self, feat, out_enc, targets, valid_ratios):
        ...
        Args:
            ...
            out_enc (Tensor): Encoder model of shape
                :math:`(N, D_m, H, W)`.
    ...
    def forward_test(self, feat, out_enc, valid_ratios):
        ...
        Returns:
            Tensor: The model logit sequence tensor of shape
            :math:`(N, T, C-1)`.
        """
